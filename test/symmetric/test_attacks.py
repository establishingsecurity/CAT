...
def test_cbc_padding_oracle_single():
    plaintext = b"deadbee" * 16
    key = unhexlify("deadbeef" * 4)
    base_iv = unhexlify("beefdead" * 4)
    cipher = AES.new(key, AES.MODE_CBC, iv=base_iv)

    ciphertext = list(block_chunks(cipher.encrypt(pad(plaintext, AES.block_size)), AES.block_size))
    ...
    assert plaintext == cbc_padding_oracle(base_iv, ciphertext, oracle)
